...
def calculate_stuff(request):
        sheet1 = read_excel('ELSS schemes.xlsx')
        ...
        #df = DataFrame(sheet2)
        with urllib.request.urlopen("https://api.mfapi.in/mf/" + str(dict_schemes[option])) as url:
            data = json.loads(url.read().decode())
            df = DataFrame(data['data'])
            l_nav = l_dates = []
            #print(len(df.axes[0]))
            #print(df.loc[0 , 'date'])
            temp_str = df.loc[len(df.axes[0])-1 , 'date']
            temp_end = df.loc[0 , 'date']

            temp_str_obj = dt.datetime.strptime(temp_str , "%d-%m-%Y")
            temp_end_obj = dt.datetime.strptime(temp_end , "%d-%m-%Y")
            while(temp_str_obj<=temp_end_obj):
                dict_nav[temp_str_obj.strftime("%d-%m-%Y")]=0
            ...
            temp_end_obj = dt.datetime.strptime(temp_end , "%d-%m-%Y")
            while(temp_str_obj<=temp_end_obj):
                temp = temp_str_obj.strftime("%d-%m-%Y")
                try:
                    dict_nav[temp] = float(df.loc[temp, 'nav'])
                except KeyError:
                    pass
                temp_str_obj+=dt.timedelta(days=1)
            ...
            while(temp_str_obj<=temp_end_obj):
                ...
                if(dict_nav[temp]==0):
                    temp_str_obj-=dt.timedelta(days=1)
                    dict_nav[temp]=dict_nav[temp_str_obj.strftime("%d-%m-%Y")]
                    temp_str_obj+=dt.timedelta(days=1)
                temp_str_obj+=dt.timedelta(days=1)
            ...
            start_index = l_dates.index(start_date_obj.strftime("%d-%m-%Y"))
            end_index = l_dates.index(end_date_obj.strftime("%d-%m-%Y"))
                #print(start_index)
                #print(end_index)
            l_thirty = []
            l_yearly = []
